...
function ImageLoader({
  ...
  function rotateLeft() {
    if (rotateAngle > 0) setRotateAngle((rotateAngle - 90) % 360);
    else setRotateAngle((360 - rotateAngle - 90) % 360);
  }
  ...
  if (preview) {
    return (
      <div onClick={hideFull} data-test="toggleModal">
        {!imageData || imageData.isLoading ? (
          <Spinner>Loading...</Spinner>
        ) : (
          <img
            className="img-thumbnail pd-0 m-0 -rotate-90"
            src={imageData.data}
            alt=""
          />
        )}
    ...
    const externalCloseBtn = (
      <button
        ...
        onClick={hideFull}
      >
        <i className="fa fa-close fa-inverse" />
      </button>
    );
        ...
        <Container fluid={true}>
          <Row>
            <Col lg={8}>
              <img
                className={
                  "rotatableImage img-thumbnail pd-0 m-0 " +
                  (rotateAngle !== 0 ? "rotate" + rotateAngle : "")
                }
                src={imageData.data}
                alt=""
              />
