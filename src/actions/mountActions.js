...
export const getMountList = () => {
  return (dispatch) => {
    axiosInstance.post(urls.listMounts).then(
      (res) => {
        console.log(res);
        dispatch({
          type: GET_MOUNT_LIST,
...
export const addMount = (fs, mountPoint, mountType, vfsOpt, mountOpt) => {
  ...
  return (dispatch) => {
    axiosInstance
      .post(urls.createMount, { fs, mountPoint, mountType, vfsOpt, mountOpt })
      .then(
        (res) => {
          dispatch({
            type,
...
export const unmount = (mountPoint) => {
  ...
  return (dispatch) => {
    axiosInstance.post(urls.removeMount, { mountPoint }).then(
      (res) => {
        dispatch({
          type,
          status: REQUEST_SUCCESS,
...
export const unmountAll = () => {
  const type = REMOVE_MOUNT;
  return (dispatch) => {
    axiosInstance.post(urls.unmountAll).then(
      (res) => {
        dispatch({
          type,
          status: REQUEST_SUCCESS,
