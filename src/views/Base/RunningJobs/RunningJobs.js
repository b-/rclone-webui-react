...
function getCroppedName(name) {
  ...
  if (name.length > leftChars) {
    const croppedName =
      name.substr(0, leftChars) + " ... " + name.substr(-rightChars);
    return croppedName;
  }
  return name;
}
  ...
  return (
    <React.Fragment>
      <Row className="runningJobs">
        {size && speedAvg ? (
          <Col lg={12} className="itemName">
            {" "}
            {getCroppedName(name)} ({formatBytes(size)}) -{" "}
            {formatBytes(speedAvg)}PS{" "}
          </Col>
        ) : (
...
function JobGroup({ job, groupId }) {
  ...
  const stopJob = (e, groupId) => {
    ...
    if (groupId && groupId.indexOf("/") !== -1) {
      ...
      axiosInstance
        .post(urls.stopJob, { jobid, _async: true })
        .then(function (res) {
      ...
      if (transferring && transferring.length > 0)
        return (
          <Card className="progress-modal d-none d-sm-block">
            ...
            <CardBody
              className={
                !this.state.isShowing ? "d-none" : "progress-modal-body"
              }
