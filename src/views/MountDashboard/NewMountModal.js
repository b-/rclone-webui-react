...
const OptionFormInput = ({
  ...
  if (attr.Options || inputType === "options") {
    inputType = "select";
    examplesMap = attr.Options.map((a) => (
  ...
  return (
    <FormGroup row>
      ...
      <Col sm={7}>
        <Input
          type={inputType}
          value={currentValues[attr.Name] || attr.Default}
          name={attr.Name}
...
const NewMountModal = (props) => {
  const { buttonLabel, className, okHandle } = props;
    ...
    <div data-test="newMountModalComponent">
      ...
      <Modal isOpen={modal} toggle={toggle} className={className}>
        ...
        <ModalFooter>
          ...
          <Button data-test="cancel-button" color="secondary" onClick={toggle}>
            Cancel
          </Button>
        </ModalFooter>
      </Modal>
