  ...
  render() {
    ...
    return (
      <Modal
        isOpen={isVisible}
        toggle={this.toggle}
        data-test="newFolderComponent"
      >
        <Form onSubmit={this.handleSubmit}>
          <ModalHeader toggle={this.toggle}>
            Create New folder at {currentPath.remoteName}:{" "}
            {currentPath.remotePath}
          </ModalHeader>
          <ModalBody>
            <FormGroup row>
              <Label for="folderName" sm={5}>
                Enter the name
              </Label>
              <Col sm={7}>
                <Input
...
const mapStateToProps = (state, ownProps) => {
  ...
  if (
    currentPath &&
    state.remote.configs &&
    state.remote.configs[currentPath.remoteName]
  ) {
