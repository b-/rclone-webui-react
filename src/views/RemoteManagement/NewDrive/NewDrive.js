...
function DriveParameters({
  ...
  return <div>Select a drive type to continue</div>;
}
...
class NewDrive extends React.Component {
  ...
  startAuthentication() {
    ...
    if (this.configCheckInterval === null) {
      this.configCheckInterval = setInterval(
        ...
        NEW_DRIVE_CONFIG_REFRESH_TIMEOUT
      );
    } else {
  ...
  changeName = (e) => {
    ...
    if (driveNameIsEditable && validateDriveName(value)) {
      ...
      this.setState((prevState) => ({
        formErrors: { ...prevState.formErrors, driveName: "Cannot edit name" },
      }));
    }
  };
  ...
  componentDidMount() {
    ...
    if (drivePrefix) {
      //Edit Mode
      this.setState({
        driveName: drivePrefix,
        driveNameIsValid: true,
        driveNameIsEditable: false,
      });
      axiosInstance
            ...
            return (
              <React.Fragment key={idx}>
                <Col
                  className={
                    "text-center " +
                    (currentStepNumber === idx ? buttonActiveClassName : "")
                  }
                  md={2}
                  sm={4}
                >
                  <button
                    ...
                    onClick={() => this.setCurrentStep(idx)}
                  >
                    {idx}
                  </button>
                  <h4>{item}</h4>
  ...
  render() {
    ...
    return (
      <div data-test="newDriveComponent">
        <ErrorBoundary>
          <p>
            This 3 step process will guide you through creating a new config.
            For auto config, leave the parameters as it is.
          </p>
              ...
              <CardBody>
                ...
                <FormGroup row>
                  <Col sm={3}>
                    ...
                    <a href="https://rclone.org/commands/rclone_config/">
                      Rclone Config
                    </a>
                  </Col>
                </FormGroup>
