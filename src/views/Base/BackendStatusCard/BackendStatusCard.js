    ...
    if (mode === "card")
      return (
        <Card
          className={
            "text-center " +
            (isConnected ? "card-accent-info" : "card-accent-warning")
          }
        >
    ...
    /*Default*/ else
      return (
        <React.Fragment>
          <Button
            type="primary"
            onClick={this.toggleCheckStatus}
            className={
              isConnected
                ? "bg-info  d-none d-lg-block"
                : "bg-warning d-none d-lg-block"
            }
          >
            {" "}
            {checkStatus
...
function StatusText({ connectivityStatus, checkStatus, ipAddress, userName }) {
  ...
  if (!checkStatus) {
    statusText =
      "Not monitoring connectivity status. Tap the icon in navbar to start.";
...
export default connect(mapStateToProps, { getStatus, enableCheckStatus })(
  BackendStatusCard
);
