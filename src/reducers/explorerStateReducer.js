    ...
    switch (action.type) {
      ...
      case CHANGE_REMOTE_NAME:
        if (remoteName.indexOf("/") === 0) {
          /*The name starts with a /: local Name*/
        ...
        if (current.remotePath && current.remotePath !== "") {
          ...
          if (splitPath.length > 1)
            for (let i = 0; i < splitPath.length - 1; i++) {
              current.remotePath =
                current.remotePath + (i !== 0 ? "/" : "") + splitPath[i];
            }
        }
      ...
      case CHANGE_VISIBILITY_FILTER:
        if (action.filter) visibilityFilter = action.filter;
