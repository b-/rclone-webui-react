...
class FilesView extends React.PureComponent {
  ...
  linkShareHandle = (item) => {
    ...
    if (fsInfo.Features.PublicLink) {
      ...
      const { remoteName } = this.props.currentPath;
      createNewPublicLink(remoteName, item.Path).then(
        (res) => {
  ...
  applySortFilter = (sortFilter) => {
    ...
    if (this.props.sortFilter === sortFilter) {
      return changeSortFilter(
        containerID,
        sortFilter,
        this.props.sortFilterAscending !== true
      );
    } else {
...
const getVisibleFiles = createSelector(
  ...
  (state, props) =>
    state.remote.files[
      `${state.explorer.currentPaths[props.containerID].remoteName}-${
        state.explorer.currentPaths[props.containerID].remotePath
      }`
    ],
  (
    containerID,
